...
export interface PaperRow { id: string; name: string; subject: string; paper_type: string; problem_count: number; created_at: string; is_favorite: boolean; is_shared: boolean; maker: string | null; unit_names: string[] }
export async function listPapers(opts: { search?: string; favorite?: boolean; shared?: boolean; trash?: boolean } = {}): Promise<PaperRow[]> {
  const supabase = await createClient();
  let q = supabase.from("paper").select("id,name,subject,paper_type,problem_count,created_at,is_favorite,is_shared,deleted_at,maker:created_by(name),paper_problem(problem_code)").order("created_at", { ascending: false });
  ...
  const { data } = await q;
  const rows = (data ?? []) as unknown as Array<{ id: string; name: string; subject: string; paper_type: string; problem_count: number; created_at: string; is_favorite: boolean; is_shared: boolean; maker?: { name?: string } | null; paper_problem?: { problem_code: string }[] }>;
